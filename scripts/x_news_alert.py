...
class RunLock:
    def __init__(self, path: Path, stale_seconds: int = 6 * 60 * 60):
        self.path = path
...
def format_time_ago(created_at: str) -> str:
    if not created_at:
        return ""
    try:
        ts_str = str(created_at).split(".")[0].rstrip("Z").replace("T", " ")
        struct = time.strptime(ts_str, "%Y-%m-%d %H:%M:%S")
        diff = int(time.time()) - calendar.timegm(struct)
        if diff < 0:
            return ""
        if diff < 60:
            return f"{diff}秒前"
...
def build_message_list_entry(
    ...
    parts = [header]
    if chinese_summary:
        parts.append(chinese_summary)
    parts.append(text)
    if tweet_url:
        parts.append(f"查看原文：{tweet_url}")
    return "\n".join(parts)
...
def process_target(
    ...
    new_tweets = dedup_tweets(target_dir, raw)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Directory for config and state files. Defaults to this scripts directory.",
    )
    ...
    try:
        ...
        if args.command == "init":
            ...
            if args.schedule_action == "show":
                ...
                print(json.dumps(schedule_config(config), ensure_ascii=False, indent=2), file=out)
